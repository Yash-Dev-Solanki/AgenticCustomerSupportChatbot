...
from tools.check_customer import check_customer
...
- When customer provides ID, use check_customer_id tool to check if the customer exists in the database.
...
TOOLS:
- check_customer_id: Checks if the provided customer ID or SSN is present in the customer database. The customer is a six digit numeric identifier  in format XXXXXX, where X is a digit from 0 to 9.The social security number is a nine digit numeric identifier in format XXX-XX-XXXX.
...
def get_welcome_agent() -> CompiledGraph:
    agent = create_react_agent(
        model= model,
        tools= [check_customer, verify_customer_id, handle_validation_failure],
        prompt= prompt,
        name= "welcome_agent",
